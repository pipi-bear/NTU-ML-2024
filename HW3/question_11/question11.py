...
for N in tqdm(N_list):
    ...
    for experiment in range(16):
        ...
        for sample in X_sample:
            input_vector = [1.0] + [sample.get(i, 0.0) for i in range(1, feature_amount + 1)]   
            X_sample_array.append(input_vector)
        
        # subsubaim: convert X_sample_array to a matrix, and y_sample to an array
        X_mat = np.array(X_sample_array, dtype=float)
        y_array = np.array(y_sample, dtype=float)
        ...
        X_out = np.array([[1.0] + [X[i].get(j, 0.0) for j in range(1, feature_amount + 1)] for i in out_sample_indices])
